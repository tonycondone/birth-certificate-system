...
class SettingsController
{
        ...
        try {
            ...
            $stmt = $this->db->prepare("
                SELECT id, first_name, last_name, email, phone_number, role, created_at, updated_at, 
                       email_verified_at, last_login_at, status
                FROM users 
                WHERE id = ?
            ");
            $stmt->execute([$userId]);
    ...
    public function updateProfile()
    {
        ...
        try {
            ...
            $stmt = $this->db->prepare("
                UPDATE users 
                SET first_name = ?, last_name = ?, phone_number = ?, updated_at = NOW()
                WHERE id = ?
            ");
    ...
    {
        error_log("SettingsController::changePassword called");
        ...
        try {
            ...
            error_log("Password update success: " . ($success ? 'true' : 'false'));
    ...
    public function getApplications()
    {
        ...
        try {
            ...
            // Add can_delete property and child_name for each application
            $processedApplications = array_map(function($app) {
                $deletableStatuses = ['draft', 'rejected', 'cancelled'];
                $app['can_delete'] = in_array($app['status'], $deletableStatuses);
                $app['child_name'] = trim($app['child_first_name'] . ' ' . $app['child_last_name']);
                $app['submitted_at'] = $app['created_at']; // For compatibility with frontend
                return $app;
            }, $applications);
    ...
    {
        error_log("SettingsController::deleteApplication called with ID: $applicationId");
        
        if (!isset($_SESSION['user_id'])) {
            error_log("No user_id in session for deleteApplication");
            http_response_code(401);
            echo json_encode(['success' => false, 'message' => 'Unauthorized']);
            return;
        }
        ...
        if ($_SERVER['REQUEST_METHOD'] !== 'DELETE') {
            error_log("Wrong request method for deleteApplication: " . $_SERVER['REQUEST_METHOD']);
            http_response_code(405);
            echo json_encode(['success' => false, 'message' => 'Method not allowed']);
            return;
        }
        ...
        try {
            $userId = $_SESSION['user_id'];
            error_log("User $userId attempting to delete application $applicationId");
            ...
            if (!$application) {
                error_log("Application $applicationId not found for user $userId");
                http_response_code(404);
                echo json_encode(['success' => false, 'message' => 'Application not found']);
                return;
            }

            error_log("Found application with status: " . $application['status']);

            // Check if application can be deleted
            $deletableStatuses = ['draft', 'rejected', 'cancelled'];
            if (!in_array($application['status'], $deletableStatuses)) {
                error_log("Application status '{$application['status']}' is not deletable");
                http_response_code(400);
                echo json_encode([
                    'success' => false, 
            ...
            try {
                ...
                $deleteResult = $stmt->execute([$applicationId, $userId]);
                
                error_log("Delete query result: " . ($deleteResult ? 'success' : 'failed'));
                ...
                error_log("Application $applicationId deleted successfully");
